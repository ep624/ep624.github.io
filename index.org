...
Following commands will create two files: ~~/.ssh/id_rsa~ and ~~/.ssh/id_rsa.pub~.

The files are in ~.ssh~ directory.

id_rsa is your private key which much be kept save and /never/ shared with anyone.
...
* Reproducible Research Using Emacs Orgmode and R3
